...
import datetime
import data_helper
from text_cnn_solution import TextCNN
...
tf.flags.DEFINE_integer("embedding_dim", 50, "Dimensionality of word embedding (default: 300)")
tf.flags.DEFINE_string("filter_sizes", "3,4,5", "Comma-separated filter sizes (default: '3,4,5')")
tf.flags.DEFINE_integer("num_filters", 100, "Number of filters per filter size (default: 100)")
tf.flags.DEFINE_float("dropout_keep_prob", 0.95, "Dropout keep probability (default: 0.95)")
...
tf.flags.DEFINE_string("embedding_path", "glove_model.txt", "Path and name for the training file (neg examples)")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        model = models.KeyedVectors.load_word2vec_format(FLAGS.embedding_path, binary=False)
        ...
        my_embedding_matrix = np.random.uniform(-0.25, 0.25, (len(vocab_processor.vocabulary_), FLAGS.embedding_dim))
        idx = 0
        ...
        # Comment out this line, to not use word-embeddings
